...
def similarity_heatmap(similarity_file, param_x=0, param_y=1, exp_exp_similarity=None, nbins=20, phys=True, plot_params='pred'):
    """
    ...
    with h5py.File(similarity_file, 'r') as infile:
        ...
        if phys:
            if plot_params == 'truth':
                range_x = range_y = param_ranges[param_x]
            else:
                range_x = range_y = [val*1.05 for val in param_ranges[param_x]]
            truth_paramskey = 'physTruth2D'
            ...
            if plot_params == 'truth':
                ...
                range_x = range_y = (-1, 1)
            truth_paramskey = 'unitTruth2D'
            pred_paramskey = 'unitPred2D'

        plot_paramskey = truth_paramskey if plot_params == 'true' else pred_paramskey
