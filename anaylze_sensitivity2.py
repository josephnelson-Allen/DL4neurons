...
def test_sensitivity(mtype,etype,def_volts,nsamples):
    ...
    for i in range(len(param_names)):
        ...
        pkl_fn=mtype + etype + adjusted_param + str(nsamples) + '.pkl'
        try:
...
def analyze_ecds(ECDS,def_vals):
    ...
    fig.suptitle('Sensitivity analysis mean/rms ' + mtype + etype)
    ...
    fig_name = mtype + etype  + str(nsamples) + 'Analysis_sampling_size.pdf'
    fig.savefig(fig_name)
    
   
    fig1, ((ax_soma1,ax_dend1),( ax_axon1,ax4))= plt.subplots(2,2,figsize=(15,15))
    fig1.suptitle('Sensitivity analysis RMS ' + mtype + etype)
    ...
    fig_name1 = mtype + etype  + str(nsamples) + 'Analysis_sensitivity_threshold.pdf'
